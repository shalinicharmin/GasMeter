...
function convertArrayOfObjectsToCSV(array, column, additional_columns) {
  ...
  // Filter out keys that contain an array of objects
  const filteredKeys = keys.filter((key) => {
    if (
      array[0][key] &&
      Array.isArray(array[0][key]) &&
      array[0][key].length > 0 &&
      typeof array[0][key][0] === "object"
    ) {
      return false // Exclude this key
    }
    return true // Include this key
  })
...
export const DownloadPDF = (tableName, columns, tblData) => {
  ...
  const groupSize = 15 // Number of columns per group
  ...
  for (let page = 0; page < totalPages; page++) {
    ...
    const groupPdfData = tblData.map((elt, i) => {
      ...
      for (const columnName of filteredGroupColumnNames) {
        ...
        // console.log(value)
        rowData.push(value === undefined ? "Na" : value)
      }
