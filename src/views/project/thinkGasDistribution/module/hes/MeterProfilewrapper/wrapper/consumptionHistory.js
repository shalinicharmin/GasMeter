...
const ConsumptionHistory = (props) => {
  ...
  const fetchMeterConsumptionData = async (params) => {
    return await useJwt
      .getIndividualGasMeterMeterConsumptionHistoryData(params)
      .then((res) => {
        // console.log(res)
  ...
  useEffect(async () => {
    if (retry) {
      ...
      const [statusCode, response] = await fetchMeterConsumptionData(params)
      setLoader(false)
