...
const RechargeHistory = (props) => {
  ...
  const fetchMeterRechargeHistoryData = async (params) => {
    return await useJwt
      .getIndividualGasMeterMeterRechargeHistoryData(params)
      .then((res) => {
        // console.log(res)
  ...
  useEffect(async () => {
    if (retry) {
      ...
      const [statusCode, response] = await fetchMeterRechargeHistoryData(params)
      setLoader(false)
