...
const InstantBillingTable = (props) => {
  ...
  for (let i = 0; i < sorted_events.length; i++) {
    ...
    if (i === sorted_events.length - 1) {
      ...
      temp_date.setDate(1);
      sorted_events[i]['billing_start_time'] = temp_date
        .toISOString()
        .split('T')[0]
        .concat(' ', '23:59:59');
    } else {
      sorted_events[i]['billing_start_time'] =
        sorted_events[i + 1]['billing_datetime'];
    }
  }
  ...
  const tblColumn = (statehandler) => {
    ...
    for (const i in sorted_events[0]) {
      ...
      if (i !== 'id') {
        ...
        col_config.id = `${i.charAt(0).toUpperCase()}${i.slice(1)}`.replaceAll(
          '_',
          ' '
        );
        col_config.serch = i;
  ...
  return (
    <div>
      ...
      <Modal
        isOpen={xmlModal}
        toggle={() => setXmlModal(!xmlModal)}
        scrollable
        className="modal-lg"
      >
        <ModalHeader toggle={() => setXmlModal(!xmlModal)}>
          Raw XML of the data
        </ModalHeader>
        <ModalBody>
...
export default InstantBillingTable;
