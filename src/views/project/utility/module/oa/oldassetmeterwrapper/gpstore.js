...
const GpStore = props => {
  ...
  const fetchData = async params => {
    try {
      const res = await useJwt.getGPMeters(params)
      const data = await res.data
      setData(data)
      const newRowCountArr = RowCount.rowCountValue
      newRowCountArr[0] = data.length
  ...
  //  to get table response in tablecolumn
  const tblColumn = () => {
    const column = []
  ...
  return (
    <>
      ...
      <SimpleDataTable
        ...
        selectable={true}
        tblData={data}
        tableName={`Smart Meter ( Vendor Name :  ${vendor.vendor_name})`}
