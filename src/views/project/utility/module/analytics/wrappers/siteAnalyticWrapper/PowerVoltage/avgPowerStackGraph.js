...
import { Card, CardHeader, CardTitle, CardBody, CardSubtitle, InputGroup, InputGroupAddon, Button, Col, CardImg } from 'reactstrap'
import noDataFound from '@src/assets/images/svg/noDataFound.svg'

const AvgPowerStackGraph = ({ labelColor, gridLineColor, warningColorShade, lineChartDanger, lineChartPrimary, dtrSelected }) => {
  ...
  useEffect(async () => {
    if (retry) {
      ...
      if (statusCode === 401 || statusCode === 403) {
        ...
        const temp_labels = []
        const temp_avg_power_data = []
        for (let i = 0; i < temp_response.length; i++) {
          temp_labels.push(temp_response[i]['date'])
          temp_avg_power_data.push(temp_response[i]['avg_power'])
        }

        setLabels(temp_labels)
        setAvgPowerData(temp_avg_power_data)
      } else {
        setRetry(false)
        setError(true)
...
export default AvgPowerStackGraph
