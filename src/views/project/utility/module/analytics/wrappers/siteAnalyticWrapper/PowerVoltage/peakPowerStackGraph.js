...
const PeakPowerStackGraph = ({ labelColor, gridLineColor, warningColorShade, lineChartDanger, lineChartPrimary, dtrSelected }) => {
  ...
  const fetchDTRPeakPowerGraphData = async params => {
    return await useJwt
  ...
  useEffect(async () => {
    if (retry) {
      ...
      const [statusCode, response] = await fetchDTRPeakPowerGraphData(params)
      ...
      if (statusCode === 401 || statusCode === 403) {
        ...
        const temp_labels = []
        const temp_peak_power_data = []
        for (let i = 0; i < temp_response.length; i++) {
          temp_labels.push(temp_response[i]['date'])
          temp_peak_power_data.push(temp_response[i]['max_power'])
        }
...
export default PeakPowerStackGraph
