...
const MeterProfileDownloadWrapper = props => {
  ...
  const fetchData = async params => {
    return await useJwt
      .getMeterProfileDLMSDownloadRequestHistory(params)
      .then(res => {
  ...
  const requestReport = async params => {
    return await useJwt
      .postMeterProfileDLMSDownloadRequest(params)
      .then(res => {
        const status = res.status
  ...
  const onSubmitButtonClicked = async filterParams => {
    ...
    if (filterParams.hasOwnProperty('site') && filterParams['site'] && filterParams['site'] !== '') {
      params['site'] = filterParams['site']
    } else {
      params['site'] = ''
    }

    if (filterParams.hasOwnProperty('meter') && filterParams['meter'] && filterParams['meter'] !== '') {
      params['meter'] = filterParams['meter']
    } else {
      params['meter'] = ''
    }

    if (filterParams.hasOwnProperty('start_date') && filterParams['start_date'] && filterParams['start_date'] !== '') {
      params['start_date'] = filterParams['start_date']
    } else {
      params['start_date'] = ''
    }

    if (filterParams.hasOwnProperty('end_date') && filterParams['end_date'] && filterParams['end_date'] !== '') {
        ...
        {loader ? (
          ...
          !retry && (
            <SimpleDataTablePaginated
              ...
              tblData={response}
              rowCount={pageSize}
              tableName={'Meter Profile Data Download Request'}
              refresh={reloadData}
...
export default MeterProfileDownloadWrapper
