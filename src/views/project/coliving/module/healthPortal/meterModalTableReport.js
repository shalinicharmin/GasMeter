...
const MeterModalTableReport = (props) => {
  const { downFactor, rowSiteMeters, RowSiteName } = props;
  // Logout User
  ...
  // const fetchMeterList = async params => {
  //   return await useJwt
  //     .postMeterList(params)
  //     .then(res => {
  //       // console.log(res)
  ...
  //       // console.log(param)
  //       const [statusCode, response] = await fetchMeterList(param)
  ...
  //     }
  //   }
  // }, [retry, downFactor, RowSiteId])

  const tblColumn = () => {
    const column = [];
    for (const i in rowSiteMeters[0]) {
      const col_config = {};
  ...
  return (
    <>
      <DataTable
        columns={tblColumn()}
        tblData={rowSiteMeters}
        tableName={`Meter Health Report  Downfactor:{${downFactor.label}}   SiteName:{${RowSiteName} }`}
        rowCount={10}
        currentpage={page}
        ispagination
...
export default MeterModalTableReport;
