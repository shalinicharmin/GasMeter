...
import TowerConfForm from '@src/views/project/coliving/module/configuration/wrapper/towerConfForm'
import { useDispatch } from 'react-redux'
import { useHistory } from 'react-router-dom'
import authLogout from '@src/auth/jwt/logoutlogic'

const TowerConfiguration = props => {
    ...
    useEffect(async () => {
        if (props.active === '3') {
            ...
            const [statusCode, response] = await getConfiguredTowers({})
    ...
    const addUi = () => <Plus size='18' className='mr_10 cursor-pointer float-right mt_12' onClick={() => configTower()} />
            ...
            <>
                ...
                <Modal isOpen={centeredModal} toggle={() => setCenteredModal(!centeredModal)} className={`modal-dialog-centered modal-lg`}>
                    <ModalHeader toggle={() => setCenteredModal(!centeredModal)}>
                        <span className='m-0 h3'>Tower configuration</span>
                    </ModalHeader>
                    <ModalBody>
                        <TowerConfForm type={formType} selected={selectRow} sites={sites} retry={retryAgain} />
                    </ModalBody>
                </Modal>
            </>
        )}
...
export default TowerConfiguration
