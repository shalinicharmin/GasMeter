...
function ViewReport({ siteList, loadingSites }) {
  ...
  async function callApi(formData) {
    ...
    try {
      ...
      if (formData.page === 1) {
        ...
        setTotalRechargeAmount(res.data.data.result.totalRechargeAmount);
      }
  ...
  function createColumns() {
    ...
    const customWidths = {
      ...
      site_id: '240px',
      site_name: '240px',
      transaction_id: '200px',
      meter_mapping: '100px',
      user: '140px',
      recharge_date: '240px',
      hes_recharge_date: '240px',
      payment_method: '90px',
      recharge_amount: '90px',
      status: '100px',
      remarks: '100px'
    };
      ...
      {error ? (
        ...
        <DataTableV1
          ...
          extraTextToShow={
            <h5
              className={`${
                totalRechargeAmount ? 'text-success' : 'text-danger'
              } m-0`}
            >
              Total Recharge: Rs. {totalRechargeAmount || 0}
            </h5>
          }
          pointerOnHover={true}
          onRowClicked={(row) => {
