...
function ListReports({ siteList, loadingSites }) {
  loadingSites = false;
  const [refresh, setRefresh] = useState(false);
  ...
  const [selectedRow, setSelectedRow] = useState({ data: [], createdAt: '' });
  ...
  async function callApi(formData) {
    ...
    try {
      ...
      if (formData.page === 1) {
        setTotalRowsCount(res.data.data.result.length);
      }
      ...
      cell: (row, i) => {
        return (
          <Download
            onClick={() => {
              setSelectedRow(row);
              DownloadCSV(row.data, `froci-${row.createdAt}`);
            }}
            size={20}
            className="text-primary"
          />
        ...
        <DataTableV1
          ...
          tableName={
            <>
              ...
              <small>
                {' '}
                ( Reports are generated automatically on daily basis. )
              </small>
            </>
          }
