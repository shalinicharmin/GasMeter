...
const NewIssueCategory = props => {
  ...
  // Api for post request to create new issue category
  const postColivingIssueCategories = async () => {
    try {
      const res = await useJwt.postColivingIssueCategories({
        ...formValue,
        isActive: checked
      })
      if (res.status === 201) {
        ...
        toast.success(<Toast msg={'New Issue Category Created Succesfully.'} type='success' />, { hideProgressBar: true })
        props.updtTbl(true)
    ...
    <>
      <Form
        ...
        <FormGroup check inline>
          <Input type='checkbox' id='basic-cb-unchecked' name='enable' onChange={handleCheck} value={checked} className='' />
...
export default NewIssueCategory
