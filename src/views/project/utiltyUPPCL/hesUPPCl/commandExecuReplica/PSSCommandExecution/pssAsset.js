    ...
    if (totalSelectedMeterCount <= meter_command_execution_allowed) {
      if (selectedPss.length > 0) {
        ...
        if (meterList && meterList.length > 0) {
          if (meter && meter.length > 0) {
            ...
            if (temp_total_selected_meter_count <= meter_command_execution_allowed) {
              if (exists) {
                setSelectedPss([])
                setMeter([])
                toast.warning(<Toast msg={"Pss Already Exists"} type='warning' />, {
          ...
          setSelectedPss([])
          toast.warning(<Toast msg={"In Selected Pss not any Meter exist"} type='warning' />, {
            hideProgressBar: true
          })
          return false
        }
        ...
        <Button.Ripple
          ...
          onClick={() => {
            if (tableData.length > 0) {
              ...
              toast.warning(
                <Toast
                  msg={"Select PSS and Please insert at least one data in table."}
                  type='warning'
                />,
                { hideProgressBar: true }
              )
