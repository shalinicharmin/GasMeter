    ...
    if (statusCode) {
      if (statusCode === 401 || statusCode === 403) {
        ...
        if (meter_details.length > 0) {
          ...
          setLoader(false)
          toast.success(<Toast msg={"Meter Found ...."} type='success' />, {
    ...
    if (tableData.length < meter_command_execution_allowed) {
      if (tableData.length > 0) {
        ...
        if (flag) {
          // console.log(flag)
          setUserInputParameter("")
          toast.warning(<Toast msg='Meter already exist in table ....' type='warning' />, {
            hideProgressBar: true
          })
          return false
        } else {
          ...
          <Form
            onSubmit={(event) => {
              ...
              if (userInputParameter.length === 0) {
                toast.warning(<Toast msg='Enter meter Serial No. /  SC No. ...' type='warning' />, {
                  hideProgressBar: true
                })
                return false
              }
        ...
        <Button.Ripple
          ...
          onClick={() => {
            if (tableData.length > 0) {
              ...
              toast.warning(
                <Toast msg={" Please insert at least one data in table."} type='warning' />,
                { hideProgressBar: true }
              )
            }
          }}
