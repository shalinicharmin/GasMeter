...
const FeederCommand = (props) => {
  ...
  const submitCommmandRequest = async () => {
    ...
    for (const i of tableData) {
      for (const j of i.meters) {
        ...
        selectedMetersList.push({
          name: "meter",
          meter_serial: j.value,
          value: {
            pss_id: j.pss_id,
            pss_name: j.pss_name,
            feeder_id: j.feeder_id,
            feeder_name: j.feeder_name,
            site_id: j.site_id,
            site_name: j.site_name,
            meter_address: j.meter_address,
            grid_id: j.grid_id,
            meter_sw_version: j.meter_sw_version,
            meter_serial: j.value,
    ...
    if (statusCode && statusCode === 201) {
      ...
      document.getElementById("preFeederButton").click()
      resetDefault()
      setSelectedCommand("")
      setSelectedProtocol([])
      // setSelectedProtocol('')
